...
#define maximumMotorVelocity            25 //maximum motor velocity 157.08[rad/second] = 2*pi*1500[RPM]/60[seconds] or 3.5[m/s]=2*pi*roperadius(0.223[m])*1500[RPM]/60[sec] or 1500/60 = 25 [RPS]
#define VMEASURE_SAMPLE_TIME            500 //what is the timeframe for velocity measurement. [ms]
#define SLOW_VELOCITY                   0.2 //the motor use this velocity when moveing slowly from one side to the other. [m/second]
#define MAX_PWM_VALUE                   255
#define MAXIMUM_ALLOWED_PWM_VALUE_PID   240
#define NOMINAL_TRACK_LENGTH            2500 //nominal length of the track (distance between motors) [cm]
#define SAFETY_DISTANCE                 20 //safety distance from the edges in numOfmagnets
#define DELAY_BETWEEN_RUNNINGS          2000 //time to wait between runnings in [ms]
#define MAXIMUM_REQUESTED_VELOCITY      3 //the maximum velocity that can be requested on the webpage
...
#define FINITE_WAITING_TIMEOUT          50000 //finite timeout. after this time the system goes back to idle
#define MAX_NUM_OF_BOUNCES              3 //prevent multiple quick presses on the stop button
#define GENERAL_TASKS_INTERVAL          3000
...
float velLTR            = 0.1; //the velocity the user has requested for the run from left to right
float velRTL            = 0.1; //the velocity the user has requested for the run from right to left
...
int LTR_delay           = 1; //delay before slave starts to pull. in [sec]
int RTL_delay           = 1; //delay before master starts to pull. in [sec]
boolean delayHasPassed  = false; //a flag states whether the delay time has passed
boolean isCloseToEdge   = false;

//Ethernet
byte mac[] 										    	   = {0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED};
IPAddress 				 				  			       ip(192, 168, 0, 177); //ip(192, 168, 0, 177);
...
void setup(){
  ...
  Serial2.print("Targuino version "); Serial2.print(VERSION_NUMBER); Serial2.print(" build "); Serial2.println(BUILD_NUMBER);
  ...
  myPID.SetOutputLimits(0,MAXIMUM_ALLOWED_PWM_VALUE_PID);  //tell the PID to range between 0 and 255 (PWM range)
...
void loop(){
  switch (FSM_State) {
   case ZERO_STATE:
    if(OneTimeFlag){
      ...
      TarMsg =  "System in idle mode. Waiting for instructions <br />";
      OneTimeFlag = false;
    }
    switch(sysMode){
      case MS_IDLE:
      ...
      case SL_CALIBRATION:
        ...
        setWaitForRespondMode();
        break;
      
      case MS_RUNNING:
        if(areValuesLegal()){
          TarMsg =  "illigal values! Check again <br />(velocity should be higher than 0.1 and track length should be longer than 1). <br />Aborting... <br />";
          sysMode = MS_IDLE;
          break;    
        }
   ...
   case WAIT_FOR_RESPONSE:
    ...
    if(CurrentTime - PrevMsgSent > RESEND_TIMOUT && previousTxMsg != ACK_COAST && numOfresends < 3){
      sendXBEEmsg(previousTxMsg);
      numOfresends++;
      PrevMsgSent = CurrentTime;
    }
    
    if(CurrentTime - PrevMsgSent > FINITE_WAITING_TIMEOUT){ //this is after 3 resends, so in total the timout is 30 seconds
   ...
   case PULL_AND_COUNT:
    ...
    TarMsg =  "Measuring track length. Click Stop when target is near the edge <br />";
    ...
    if(timeGuard > maximumRunTime || stopSignal){ // target probably arrived to the other edge of the rope 
      coastMotor();
      TarMsg =  "Saving track length. To change track length repeat calibration process <br />";
   ...
   case PULL_SLOW:
    ...
    if(MS){ TarMsg = "Starting to pull slowly. Click Stop when target is near the edge <br />"; }
   ...
   case PULL_STATE:
    ...
    if(OneTimeFlag){
      ...
      stopSignal = false;
      OneTimeFlag = false;
      if(MS){ TarMsg = "Running from side to side. Click stop to abort <br />"; }
    }    
    ...
    if(MS && timeGuard > RTL_delay * 1000 && !delayHasPassed){ //if RTL_Delay has passed (for master) - raise the flag to start pulling
      Serial2.println("Delay passed");
      delayHasPassed = true;
    }  
    else if(!MS && timeGuard > LTR_delay * 1000 && !delayHasPassed){ //if LTR_Delay has passed (for slave) - raise the flag to start pulling
      Serial2.println("Delay passed");
      delayHasPassed = true; 
    }    
    ...
    if(delayHasPassed && checkTime(VMEASURE_SAMPLE_TIME)){  
     ...
     if(delayHasPassed && timeGuard > 1000){
      Serial2.print("desiredIn = "); Serial2.print(desiredIn); Serial2.print(" measuredIn = "); Serial2.print(measuredIn); Serial2.print(" sysIn = "); Serial2.print(sysIn); Serial2.print(" Distance: "); Serial2.println(distance_from_edge);
      tBeginMovement = CurrentTime;
     }
    break;
       
   default:
    coastMotor(); //just in case something happens...
        ...
        if(client.available()){   // client data available to read
          ...
          if (c == '\n' && currentLineIsBlank) {
              ...
              switch(out.msgType){
                ...
                if(sysMode == MS_CALIBRATION){ //eliminate multiple presses effect
                    break;
                  }
                  if(!isSlaveConnected){
                    TarMsg =  "Calibration sequence has not commenced - no connection to slave <br />";
                    Serial2.println(TarMsg);
                    break;
                  }
                  TarMsg =  "Calibration sequence - loading... <br />";
                ...
                case UPDATE_BEGIN_RUNNING:
                  if(sysMode == MS_RUNNING){ //eliminate multiple presses effect
                    break;
                  }
                  if(!isSlaveConnected){
                    TarMsg =  "Running sequence has not commenced - no connection to slave <br />";
                    Serial2.println(TarMsg);
                    break;
                  }
                  sysMode = MS_RUNNING;
                  TarMsg =  "Running sequence - loading... <br />";
                ...
                case UPDATE_STOP:
                  ...
                  if(FSM_State == WAIT_FOR_RESPONSE){
                    if(sysMode == MS_CALIBRATION){
                      TarMsg += "Stop button pressed. Preparing to measure length...<br />";
                      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Stop button pressed while master is waiting in calibration mode. nB="); Serial2.println(nBounces);
                    }
                    else if(sysMode == MS_RUNNING){
                      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Stop msg while master is waiting in RUNNING mode. nB="); Serial2.println(nBounces);
                      TarMsg =  "To stop running sequence tap the STOP button at least three times <br />";
                    }
                   sendXBEEmsg(FINISH_PULLING);
                  }
                  else if(FSM_State == PULL_SLOW || FSM_State == PULL_AND_COUNT || FSM_State == PULL_STATE){
                    Serial2.print(float(CurrentTime / 1000)); Serial2.println(" Stop msg while master in pullSlow / PullAndCount / PullState!");
                    stopSignal = true; 
                  }

                  nBounces++;

                  if( nBounces > MAX_NUM_OF_BOUNCES){
                    TarMsg =  "Stop button was tapped multiple times - ABORTING, Please wait...<br />";
                    coastMotor();
                    sendXBEEmsg(STOP_MOTOR);
                    Serial2.print(float(CurrentTime / 1000)); Serial2.print(" "); Serial2.print(nBounces); Serial2.println(" bounces were detected");
                    setWaitForRespondMode();
                    nBounces = 0;
                  }
                  break;
                ...
                case OPEN_FILE:
                  Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Sending requested file..."); Serial2.println(out.fileToOpen);
                  sendHTTPResponse(out.httpResponse, client);
                  sendFile(out.fileToOpen, client);
                  break;
  ...
  if(CurrentTime - PreviousTimeForGeneralTasks > GENERAL_TASKS_INTERVAL){ 
    //A. zeroize nBounces after 5 seconds - resets multpile stop presses
    nBounces = 0;
    PreviousTimeForGeneralTasks = CurrentTime;

    //B. display some debug data on serial2
    if(FSM_State == ZERO_STATE){
      ...
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" OutMsgIndex: "); Serial2.print(MsgCounter); Serial2.print(" InMsgIndexFromOtherSide: "); Serial2.println(MsgCounterAtDest);        
    }
...
void SerialProcess(){
  ...
  //Serial2.print(float(CurrentTime / 1000)); Serial2.print(" buffer size: "); Serial2.print(Serial.available()); Serial2.print(" bytes. "); Serial2.print("tempByte: "); Serial2.println(tempByte,HEX);
    
  if(tempByte == STOP_BYTE || msgIndex > 7){ //maximum value of msgIndex is 4 (last index of recBuff) 
    isMsg = false;
    msgIndex = 0;
    processedMSG = procRecMsg();
    inMsgCount = recBuff[3];
  }
  else if(isMsg){
  ...
  if(processedMSG == previousRxMsg){ //drop double messages
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Message number "); Serial2.print(inMsgCount); Serial2.print(" dropped: "); Serial2.println(processedMSG, HEX);
      nDroppedMsgs++;
      numOfKAmsgs = 0;
      return;
      } 
  else if(processedMSG != previousRxMsg){
  ...
  switch(processedMSG){
    case ACK_KA: //keep alive ack
      ...
      MsgCounterAtDest = recBuff[4];
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Keep alive ack, msg index is "); Serial2.println(inMsgCount);
      if( abs(MsgCounter - MsgCounterAtDest) % 255 > 3 && MsgCounter > 3){
        msgIndDiff = abs(MsgCounter - MsgCounterAtDest) % 255;
        sendXBEEmsg(CLEAR_BUFFER);
        Serial2.print(float(CurrentTime / 1000)); Serial2.println(" clear buffer request sent");
      }
      break;
    ...
    case GOTO_CALIBRATION:
      ...
      sendXBEEmsg(ACK_CALIB);
      setWaitForRespondMode();
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" slave: gotocalibration msg received. sent: "); Serial2.println(ACK_CALIB,HEX);
      break;
    ...
    case KEEP_ALIVE:
      MsgCounterAtDest = recBuff[4];
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Keep alive received, msg index is "); Serial2.println(inMsgCount);
      if( abs(MsgCounter - MsgCounterAtDest) % 255 > 3 && MsgCounter > 3){
        msgIndDiff = abs(MsgCounter - MsgCounterAtDest) % 255;
        sendXBEEmsg(CLEAR_BUFFER);
        Serial2.println("clear buffer request sent");
      }
      else{
        sendXBEEmsg(ACK_KA);
      } 
    ...
    case ACK_CALIB:
      coastMotor();
      sendXBEEmsg(START_SLOW_PULLING);
      setWaitForRespondMode();
    ...
    case ACK_RUN:
      ...
      TarMsg =  "Slave is pulling before starting to run. <br />Press Stop when target is close to the edge <br />";
      break;
    ...
    case ACK_COAST: //only master motor get ack_coast
      if(FSM_State == MS_IDLE){ break;}
      switch(sysMode){
        case MS_CALIBRATION:
          desiredIn = (double) convertMagnetsToPWM(convertVelToMagnets(SLOW_VELOCITY), VMEASURE_SAMPLE_TIME);
          FSM_State = PULL_AND_COUNT;
          stopSignal = false;
          OneTimeFlag = true;
          tBeginMovement = CurrentTime;
          Serial2.print(float(CurrentTime / 1000)); Serial2.print(" ack coast received in CALIBRATION mode "); Serial2.println(sysMode, HEX);
          break;
        case MS_RUNNING:
          if(nBounces > MAX_NUM_OF_BOUNCES){ break;}
          desiredIn = (double) convertMagnetsToPWM(convertVelToMagnets(velRTL), VMEASURE_SAMPLE_TIME);
          FSM_State = PULL_STATE;
          stopSignal = false;
          OneTimeFlag = true;
          tBeginMovement = CurrentTime;
          Serial2.print(float(CurrentTime / 1000)); Serial2.print(" ack coast received in RUNNING mode "); Serial2.println(sysMode, HEX);
          TarMsg = "Master is pulling now. Waiting for it to finish";
          break;
        default:
    ...
    case DONE_PULLING:
      ...
      if(sysMode == MS_RUNNING){
        TarMsg =  "Running sequence has started. Press stop to go back to idle mode <br />";
      }
      break;  
      
    case START_SLOW_PULLING:
    ...
    case ACK_PULL:
      TarMsg = "Slave is pulling now. Waiting for it to finish";
      setWaitForRespondMode();
      break;

    case START_PULLING:
      if(sysMode != SL_RUNNING){
        Serial2.println("StartPulling error - sysMode is not in SL__RUNNING mode");
        break;
      }
      else{
        desiredIn = (double) convertMagnetsToPWM(convertVelToMagnets(velLTR), VMEASURE_SAMPLE_TIME);
        FSM_State = PULL_STATE;
        stopSignal = false;
        tBeginMovement = CurrentTime;
    ...
    case TRACK_LENGTH_UPDATE:
      lTrack = (int) recBuff[4] * 255 + (int) recBuff[5];
      Serial2.print(float(CurrentTime / 1000)); Serial2.print(" lTrack received: "); Serial2.println(lTrack); 
...
void MeasureVelocityInPWM(unsigned int tDomain){ 
  
  long tDomainLong = (long) tDomain;

  measuredIn = (double) convertMagnetsToPWM(Revolutions, tDomainLong);
...
boolean sendFile(String Tfilename, EthernetClient cl){

  char cFilename[Tfilename.length()+1];
  Tfilename.toCharArray(cFilename, Tfilename.length()+1);

  TServerFile = SD.open(cFilename);        // open web page file
  if(TServerFile) {
    ...
    Serial2.print("ERROR - Can't open "); Serial2.println(cFilename);
    return 1;
  }
  }
...
void sendXBEEmsg(byte buff){
  ...
  else if(buff == ACK_KA || buff == KEEP_ALIVE){
    msgToSend[5] = inMsgCount;
  }
  else if(buff == CLEAR_BUFFER){
    msgToSend[5] = msgIndDiff;
  }
  else{ msgToSend[5] = 0; }
  ...
  delay(40); //delay due to RO parameter on XBEE
  
  Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Sent message number "); Serial2.print(MsgCounter); Serial2.print(": "); Serial2.print(buff, HEX); Serial2.print(" inMsgCount: "); Serial2.print(inMsgCount); Serial2.println(" ");
...
boolean areValuesLegal(){ //check wether the values for the velocities are correct
  if(velRTL > MAXIMUM_REQUESTED_VELOCITY || velLTR > MAXIMUM_REQUESTED_VELOCITY){ return 1;}
  else if(velRTL < 0.1 || velLTR < 0.1){ return 1;}
  else if(lTrack < 1){ return 1;}
  else{ return 0;}
}

byte convertVelToMagnets(float inVel){

  float preciseCountedMagnets =  inVel * numOfMagnets * VMEASURE_SAMPLE_TIME * 100 / ( 2 * pi * ropeRadius * 1000 ); // for expected value calculation we can use the constant VMEASURE_SAMPLE_RATE
  
  return mround(preciseCountedMagnets);
}

byte convertMagnetsToPWM(byte inMagnets, long tDomainDiff){

  //Serial2.print(CurrentTime); Serial2.print(" ##DEBUG rev: "); Serial2.print(Revolutions); Serial2.print(" Denum: "); Serial2.print(tDomainDiff * numOfMagnets * maximumMotorVelocity); Serial2.print(" deltaT: "); Serial2.println(deltaT);

  float PWMout = float(inMagnets) * 255 * 1000 / (tDomainDiff * numOfMagnets * maximumMotorVelocity); // for real-time calculation we need to use the real deltaT
...
struct parsedResult parseHTTPmsg(char *msg){
 ...
 //look for status
  ...
  if(ind){
   ...
   //look for targuino.CSS file
  ind = StrContains(msg, CSS_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = CSS_FILENAME;
    output.httpResponse = 'c';
    return output;
  }

  //look for targuino.js file‎
  ind = StrContains(msg, JAVASCRIPT_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = JAVASCRIPT_FILENAME;
    output.httpResponse = 'j';
    return output;
  }
  
  //look for jpg file
  ind = StrContains(msg, JPG_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = JPG_FILENAME;
    output.httpResponse = 'g';
    return output;
  }

  //look for htm file
  ind = StrContains(msg, WEBSITE_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = WEBSITE_FILENAME;
    output.httpResponse = 'h';
    return output;
  }

  //look for png file
  ind = StrContains(msg, ICO_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = ICO_FILENAME;
    output.httpResponse = 'i';
    return output;
  }

  //look for jquery file
  ind = StrContains(msg, JQUERY_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = JQUERY_FILENAME;
    output.httpResponse = 'j';
    return output;
  }

  //look for uislider js file
  ind = StrContains(msg, JS_UISLIDER_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = JS_UISLIDER_FILENAME;
    output.httpResponse = 'j';
    return output;
  }

  //look for uislider css file
  ind = StrContains(msg, CSS_UISLIDER_FILENAME);
  if(ind){
    output.msgType = OPEN_FILE;
    output.fileToOpen = CSS_UISLIDER_FILENAME;
    output.httpResponse = 'c';
    return output;
  }
...
void printXML(EthernetClient cl){
  ...
  cl.print("<SysMode>"); 
  if(sysMode == MS_IDLE){ cl.print("Idle"); }
  else if(sysMode == MS_CALIBRATION){ cl.print("Calibration"); }
  else if(sysMode == MS_RUNNING){ cl.print("Running"); } 
  cl.println("</SysMode>");
...
void cleanXBEE(){
  Serial2.print(float(CurrentTime / 1000)); Serial2.print(" forwarding incoming msgs "); Serial2.println(Serial.available());
  int i = 0;
  while(Serial.available() > 0 && i < recBuff[4]){
    tempByte = Serial.read();
    Serial2.print(tempByte,HEX); Serial2.print(" "); Serial2.println(i);
    i++;
  }
}