...
#define SLOW_VELOCITY           0.6 //the motor use this velocity when moveing slowly from one side to the other. [m/second]
...
void loop(){
  switch (FSM_State) {
    ...
    switch(sysMode){
      ...
      case MS_RUNNING:
        if(areVelocitiesLegal()){
          TarMsg =  "illigal values! Check velocity (velocity cannot be under 0.1). Aborting... <br />";
          sysMode = MS_IDLE;
          break;    
        }
   ...
   case WAIT_FOR_RESPONSE:
    ...
    if(numOfresends > FINITE_WAITING_TIMEOUT){ //this is after 3 resends, so in total the timout is 30 seconds
   ...
   case PULL_SLOW:
    ...
    if(MS){ TarMsg = "Starting to pull slowly. Click Stop when target is near the edge ƒ<br />"; }
              ...
              switch(out.msgType){
                ...
                case UPDATE_STOP:
                  if(FSM_State == WAIT_FOR_RESPONSE){
                    ...
                    Serial2.print(float(CurrentTime / 1000)); Serial2.println("Stop msg while pullSlow / PullAndCount / PullState!");
                    stopSignal = true; 
                  }

                  if(hasStopBeenClickedOnce){
                    coastMotor();
                    sendXBEEmsg(STOP_MOTOR);
                    Serial2.print(float(CurrentTime / 1000)); Serial2.println("General stop msg! two stop presses were detected");
                    setWaitForRespondMode();
                    hasStopBeenClickedOnce = false;
                    break;
                  }

                  hasStopBeenClickedOnce = true; //to determine whether we clicked stop once or twice
                  break;
  ...
  if(CurrentTime - PreviousTimeForGeneralTasks > 5000){ 
    //A. zeroize hasStopBeenClickedOnce after 5 seconds - resets two stop presses
    hasStopBeenClickedOnce = false;
