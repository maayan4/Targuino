...
//Other Constants
#define pi                      3.14159
#define radius                  6.5 //[cm] radi‎‎‎us of the wheel. represents velocity at the end point
...
#define RESEND_RESPONSE_TIMOUT    10 //timeout for a response from the other side
...
unsigned long wasFirstStop                    = 0; //for two stop button presses

//XBEE variables
byte msgToSend[7]              = {START_BYTE, 0, 0, 0, 0, 0,  STOP_BYTE};
byte recBuff[5]                = {0, 0, 0, 0, 0};
byte previousMsg               = 0xFF; 
...
void loop(){
  switch (FSM_State) {
   ...
   case WAIT_FOR_RESPONSE:
    ...
    if(CurrentTime - PrevMsgSent > RESEND_TIMOUT && numOfresends < 3){
      sendXBEEmsg(previousMsg);
      numOfresends++;
      PrevMsgSent = CurrentTime;
    }

    if(numOfresends > RESEND_RESPONSE_TIMOUT || stopSignal){
      Serial2.println("wait for respons: respone-timout");
      coastMotor();
   ...
   case PULL_SLOW:
    ...
    if(checkTime(VMEASURE_SAMPLE_TIME)){  
      ...
      movemotor(DIR,sysIn,MAX_PWM_VALUE);
      Serial2.print("desiredIn = "); Serial2.print(desiredIn); Serial2.print("measuredIn = "); Serial2.print(measuredIn); Serial2.print("sysIn = "); Serial2.print(sysIn); Serial2.print("Rev = "); Serial2.println(Revolutions);
     }
  ...
  if(CurrentTime - wasFirstStop > 3000){ //zeroize wasfirststop after 3 seconds - resets two stop presses
    wasFirstStop = 0;
  }
              ...
              switch(out.msgType){
                ...
                case UPDATE_STOP:
                  ...
                  if(FSM_State == WAIT_FOR_RESPONSE){
                    sendXBEEmsg(STOP_MOTOR);
                    Serial2.println("General stop msg while waiting!");
                  }
                  if(FSM_State == PULL_SLOW || FSM_State == PULL_AND_COUNT){
                    stopSignal = true; 
                  }
                  if(FSM_State == PULL_STATE){
                    if(wasFirstStop){
                      sendXBEEmsg(STOP_MOTOR);
                      Serial2.println("General stop msg! two stop presses were detected");
                    }
                    else{
                      stopSignal = true; 
                      wasFirstStop = CurrentTime;
                    }
                  }
                  if(sysMode == MS_IDLE){
  ...
  if(sumsum > 4000){
    ...
    if(MS){   Serial2.print(" RTL_delay: "); Serial2.print(RTL_delay); Serial2.print(" RTL_Velocity: "); Serial2.println(velRTL); }
    else{     Serial2.print(" LTR_delay: "); Serial2.print(LTR_delay); Serial2.print(" LTR_Velocity: "); Serial2.println(velLTR); }
...
void serialEvent() {
  ...
  if(Serial.available() > 0){
    ...
    incomingByte = true;
    //Serial2.println(tempByte, HEX);
    }  
  
  if(tempByte == STOP_BYTE || msgIndex > 4){ //maximum value of msgIndex is 4 (last index of recBuff) 
    isMsg = false;
    msgIndex = 0;
    processedMSG = procRecMsg();
    clrXBEErecBuff();      
    Serial2.print(float(CurrentTime / 1000)); Serial2.print(" Proccessed message is: "); Serial2.println(processedMSG,HEX);
  }
  ...
  if(tempByte == START_BYTE){ isMsg = true;}
  ...
  switch(processedMSG){
    ...
    case LTR_VELOCITY_UPDATE:
      previousMsg = LTR_VELOCITY_UPDATE; //preparing for velocity values
      break;
    ...
    case LTR_DELAY_UPDATE:
      previousMsg = LTR_DELAY_UPDATE; //preparing for velocity values
      break;

    case STOP_MOTOR:

      if(FSM_State == PULL_SLOW || FSM_State == PULL_AND_COUNT || FSM_State == PULL_STATE){
        stopSignal = true;
        break;
      }      
      else{
    ...
    case START_PULLING:
      switch(sysMode){
        ...
        case MS_RUNNING:
          ...
          RunLength = 0;
          tBeginMovement = CurrentTime;
          Serial2.print("starting to pull");
          break;
