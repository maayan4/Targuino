...
void loop(){
  switch (FSM_State) {
   case ZERO_STATE:
    ...
    switch(sysMode){
      case MS_IDLE:
        ...
        if(randnum < 33){      whatToSend = SEND_VELOCITY;}
        else if(randnum > 67){  whatToSend = SEND_DELAY;}
   ...
   case WAIT_FOR_RESPONSE:
    ...
    if(CurrentTime - PrevMsgSent > RESEND_RESPONSE_TIMOUT){ //this is after 3 resends, so in total the timout is 30 seconds
...
void serialEvent() {
  ...
  switch(processedMSG){
    case ACK_KA: //keep alive ack
      ...
      if( abs(MsgCounter - recBuff[4]) > 3 && MsgCounter > 3){
    ...
    case CLEAR_BUFFER:
      Serial2.println("Cleaning receive buffer...");
