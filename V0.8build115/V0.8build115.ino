...
#define maximumMotorVelocity    25 //maximum motor velocity 157.08[rad/second] = 2*pi*1500[RPM]/60[seconds] or 5.575[m/s]=roperadius(0.223[m])*1500[RPM]/60[sec] or 1500/60 = 25 [RPS]
...
int nDroppedMsgs                     = 0;
unsigned PreviousTimeForGeneralTasks = 0; //this is a timer in loop() for general tasks
...
void loop(){
  switch (FSM_State) {
    ...
    switch(sysMode){
      ...
      case MS_RUNNING:
        if(areVelocitiesLegal()){
          TarMsg = "illigal values! Check velocity (velocity cannot be under 0.1). Aborting...";
          sysMode = MS_IDLE;
          break;    
        }
   ...
   case WAIT_FOR_RESPONSE:
    ...
    if(CurrentTime - PrevMsgSent > RESEND_TIMOUT && previousTxMsg != ACK_COAST){
      sendXBEEmsg(previousTxMsg);
      numOfresends++;
      PrevMsgSent = CurrentTime;
    }
    
    if(numOfresends > MAXIMUM_NUM_OF_RESENDS){ //this is after 3 resends, so in total the timout is 30 seconds
   ...
   case PULL_AND_COUNT:
    ...
    TarMsg = "Measuring track length. Click Stop when target is near the edge";
    ...
    if(timeGuard > maximumRunTime || stopSignal){ // target probably arrived to the other edge of the rope 
      coastMotor();
      TarMsg = "Saving track length. To change track length repeat calibration process";
   ...
   case PULL_SLOW:
    ...
    if(MS){ TarMsg = "Starting to pull slowly. Click Stop when target is near the edge"; }
   ...
   case PULL_STATE:
    ...
    if(OneTimeFlag){
      ...
      stopSignal = false;
      OneTimeFlag = false;
      if(MS){ TarMsg = "Running from side to side. Click stop to abort"; }
    }    
  ...
  if(CurrentTime - PreviousTimeForGeneralTasks > 5000){ 
    //A. zeroize hasStopBeenClickedOnce after 4 seconds - resets two stop presses
    hasStopBeenClickedOnce = false;
