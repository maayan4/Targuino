...
void serialEvent2() {
  ...
  if(!MS){
      ...
      // get the new byte:
      temp_c = (char) Serial2.read();
		  temp_i = Serial2.parseInt();
      Serial.print(temp_c); Serial.println(temp_i);
	   ...
	   switch(temp_c){
        ...
        if(temp_i == 999){ sysIn = 0; }
        Serial.print("calc_sysin="); Serial.println(sysIn);
        movemotor(DIR,sysIn,MAX_PWM_VALUE);
	      break;
	    default:
	      break;    
...
void stopMotors(){
  ...
  movemotorB(DIR,999,MAX_PWM_VALUE); //not a mistake! stopMotor message should be transmitted twice just in case
