...
#define RESPONSE_TIMEOUT    70000 //timeout for a response from the other side
...
void setup(){
  ...
  Serial.flush(); //clean Serial transmit b
  while(Serial.available()){ Serial.read(); } //clean Serial receive buffer
   ...
   case ZERO_STATE:
    ...
    switch(sysMode){
      case MS_IDLE:
        ...
        if(random(0,10) < 3){      whatToSend = SEND_VELOCITY;}
        else if(random(0,10) > 7){ whatToSend = SEND_DELAY;}
          ...
          if (c == '\n' && currentLineIsBlank) {
              ...
              switch(out.msgType){
                ...
                case UPDATE_STOP:
                  ...
                  if(FSM_State == PULL_SLOW_VELOCITY || FSM_State == PULL_AND_COUNT || FSM_State == PULL_STATE || FSM_State == PULL_SLOWLY){
                    stopSignal = true;
                  }
                  if(sysMode == MS_IDLE){
...
void serialEvent() {
  ...
  switch(processedMSG){
    ...
    case 0xFF:
      //no message received or message totally corrupted
      break;
...
byte procRecMsg(){
  ...
  //Serial2.print("cmpr1 = "); Serial2.print(cmpr1); Serial2.print(" cmpr2 = "); Serial2.print(cmpr2); Serial2.print(" cmpr3 = "); Serial2.println(cmpr3); 
  
  if(cmpr1 && cmpr2 && cmpr3){ return 0xFF; } //message is totally corrupted. all of the messages are different from each other
